...
import uk.gov.hmrc.gform.models.{ AddToListUtils, DataExpanded, FastForward, FormModel, FormModelBuilder, ProcessData, ProcessDataService, Repeater, SectionSelectorType, Singleton }
        ...
        processResponseDataFromBody(request, formModelOptics.formModelRenderPageOptics.formModel) {
          requestRelatedData => variadicFormData =>
            ...
            def validateAndUpdateData(
              ...
              for {
                ...
                res <- {
                  ...
                  //println("[validateAndUpdateData] v: " + (v))
                  ...
                  /* println("formData: ")
                   * formData.fields.foreach(println) */
                  /* println("formData2: ")
                   * formData2.fields.foreach(println)
                   * println("====================") */
                  /* val oldDataFiltered = oldData.data.filterNot {
                   *   case (k, v) =>
                   *     formData.fields.toList.exists(a => a.id === k)
                   * } */

                  /* println("oldData        : " + (oldData))
                   *
               println("oldDataFiltered: " + (oldDataFiltered))
                   *
            ...
            def processAddGroup(processData: ProcessData, modelComponentId: ModelComponentId): Future[Result] = {
              ...
              //val (updatedData, anchor) = addNextGroup(maybeGroupFc, processData.formModelOptics, lookupExtractors)

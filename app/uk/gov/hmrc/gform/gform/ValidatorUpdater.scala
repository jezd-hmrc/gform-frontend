...
import uk.gov.hmrc.gform.sharedmodel.formtemplate.{ BankAccoutnModulusCheck, FormComponentId, FormCtx, HmrcRosmRegistrationCheckValidator, Validator }
...
class ValidatorUpdater(validator: Validator, index: Int, baseIds: List[FormComponentId]) {
  ...
  def updated: Validator = validator match {
    ...
    case v: BankAccoutnModulusCheck =>
      v.copy(
        errorMessage = expandSmartString(v.errorMessage),

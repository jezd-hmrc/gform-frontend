...
class ExprUpdater(expr: Expr, index: Int, baseIds: List[FormComponentId]) {
  ...
  def updated = expandExpr(expr)
}

object ExprUpdater {
  def apply(expr: Expr, index: Int, baseIds: List[FormComponentId]) = new ExprUpdater(expr, index, baseIds)
}

...
case class RecData[S <: SourceOrigin](
  ...
  val cleared = variadicFormData -- recalculatedTaxPeriod.keySet.map(_.fcId.modelComponentId)
}
...
object RecData {
  ...
  def fromData[S <: SourceOrigin](variadicFormData: VariadicFormData[S]): RecData[S] =
    RecData(variadicFormData, Map.empty[RecalculatedTaxPeriodKey, IdNumberValue])
}

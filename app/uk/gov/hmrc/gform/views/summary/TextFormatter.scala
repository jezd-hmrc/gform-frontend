...
import uk.gov.hmrc.gform.validation.{ FormFieldValidationResult, MultiFieldId }
    ...
    def getValue(formComponent: FormComponent): String = formComponent match {
      ...
      case IsUkSortCode(sortCode) =>
        sortCode
          ...
          .map(modelComponentId => validationResult.getCurrentValue(MultiFieldId.whatIsThis(modelComponentId)))
          .toList
          .mkString("-")
      ...
      case IsDate(date) =>
        ...
        date
          ...
          .map(
            modelComponentId =>
              monthToString(
                modelComponentId.atom,
                validationResult.getCurrentValue(MultiFieldId.whatIsThis(modelComponentId))))
          .toList
          .mkString(" ")
      case _ => currentValue
    }

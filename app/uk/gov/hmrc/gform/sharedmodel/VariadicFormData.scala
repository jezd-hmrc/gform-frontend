...
object VariadicFormData {
  ...
  // there is no corresponding ModelComponentId in the template.
  // The only use of the FormTemplate is to determine which branch of VariadicValue each ModelComponentId should use,
   ...
   * ): VariadicFormData[S] =
   *   buildFromMongoData(listVariadicFormComponentIds(formModel), data) */
  ...
  def buildFromMongoData[S <: SourceOrigin](
    ...
    val multiValueIds
      : Set[BaseComponentId] = formModel.allMultiSelectionIds.map(_.baseComponentId) //.map(_.extractVariadicValueId)
  ...
  /* def listVariadicFormComponentIds(template: FormTemplate): Set[ModelComponentId] = {
   ...
   *       case s: AddToList =>
   *         s.pages.foldMap(listVariadicFormComponentIds(_)) + ModelComponentId.Simple(s.addAnotherQuestion.id)
   *     }
   * } */

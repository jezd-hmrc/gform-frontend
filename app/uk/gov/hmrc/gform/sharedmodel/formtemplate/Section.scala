...
object Section {
  ...
  case class RepeatingPage(page: Page[Basic], repeats: Expr) extends Section {
    val allIds: List[FormComponentId] = page.fields.map(_.id)
  }
  ...
  case class AddToList(
    ...
    val allIds: List[FormComponentId] = {
      addAnotherQuestion.id :: pages.toList.flatMap(page =>
        page.fields.map(_.id) ++ page.fields.flatMap(_.childrenFormComponents.map(_.id)))
    }
  }

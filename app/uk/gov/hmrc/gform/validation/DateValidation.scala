...
class DateValidation[D <: DataOrigin](formModelVisibilityOptics: FormModelVisibilityOptics[D])(
  ...
  private def validateConcreteDateWithMessages(
    ...
    validateInputDate2(formComponent)
      .andThen { inputDate =>
  ...
  private def validateDateFieldWithMessages(
    ...
    val validateOtherDate = validateInputDate2(otherFieldValue)
    val validatedThisDate = validateInputDate2(fieldValue)

    validateOtherDate.andThen { otherLocalDate =>
      validatedThisDate.andThen { thisLocalDate =>
        if (beforeAfterPrecisely.datePredicate(thisLocalDate, otherLocalDate)) validationSuccess
        else {
  ...
  private def validateInputDate2(
    formComponent: FormComponent

...
object ValidationUtil {
  ...
  // TODO JoVl Why this exists and why there is +- same version in Field.scala?
  ...
  def evaluateValidationResult[D <: DataOrigin](
    ...
    /* println("[evaluateValidationResult] atomicFields: ")
     * atomicFields.foreach(println) */
    ...
    def matchComponentType(formComponent: FormComponent): FormFieldValidationResult = formComponent match {
      case IsMultiField(_) =>
        ...
        val dataMap = (valWithoutSuffixResult :: valSuffixResult).map { kv =>
          //println("[evaluateValidationResult] kv._1: " + (kv._1))
          //println("[evaluateValidationResult] kv._2: " + (kv._2))
          MultiFieldId.whatIsThis(kv._1) -> kv._2
        }.toMap

        /* println("[evaluateValidationResult] dataMap: ")
        ...
        gFormErrors.get(atomicFcId) match {
          ...
          case None =>
            val optionalData = formModelVisibilityOptics.data.one(atomicFcId).map { selectedValue =>
              selectedValue.map { _ =>
                MultiFieldId
                  .whatIsThis(atomicFcId) -> FieldOk(formComponent, dataGetter(atomicFcId).headOption.getOrElse(""))
              }.toMap

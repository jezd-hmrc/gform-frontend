...
object DateValidationLogic {
  ...
  def incorrectDateMessage(
    ...
    val yearString = concreteDate.year match {
      case Year.Exact(year) if concreteDate.month.isAny && concreteDate.day.isAny => messages("date.year") + " " + year.toString
      ...
      case Year.Next if concreteDate.month.isAny && concreteDate.day.isAny        => messages("date.year") + " " + getNextYear.toString
      case Year.Next                                                              => messages("date.inYear", getNextYear.toString)
      case Year.Previous if concreteDate.month.isAny && concreteDate.day.isAny    => messages("date.year") + " " + getPreviousYear.toString
    ...
    val monthString = concreteDate.month match {
      case Month.Exact(month) if concreteDate.day.isAny             => messages(getMonthName(month))
      case Month.Exact(month) if !concreteDate.day.isExact          => messages("date.inMonth", getMonthName(month))

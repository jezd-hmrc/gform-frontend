...
class FormModelBuilder[E, F[_]: Functor](
  ...
  private def toCurrentData(
    ...
    val atomicFcId = fcId.modelComponentId // TODO JoVl implement expression support for multivalue fields ie. date, sortcode, address
    expressionResult match {
      case ExpressionResult.Empty      => VariadicFormData.one[SourceOrigin.Current](atomicFcId, "")
      ...
      case ExpressionResult.NumberResult(bigDecimal) =>
        VariadicFormData.one[SourceOrigin.Current](atomicFcId, bigDecimal.toString)
      case ExpressionResult.IntResult(integer) =>
        VariadicFormData.one[SourceOrigin.Current](atomicFcId, integer.toString)
      case ExpressionResult.StringResult(value) => VariadicFormData.one[SourceOrigin.Current](atomicFcId, value)
      case ExpressionResult.OptionResult(value) =>
        VariadicFormData.many[SourceOrigin.Current](atomicFcId, value.map(_.toString))
    }
  }
        ...
        visibleTypedExprs.foldMap {
          case (fcId, typedExpr) =>
            evaluationResults.get(typedExpr).fold(VariadicFormData.empty[SourceOrigin.Current]) { expressionResult =>
              toCurrentData(fcId, expressionResult)
            }
        }
    ...
    formModel.flatMap {
      ...
      case Repeater(title, description, shortName, includeIf, addAnotherQuestionFc, index, source) =>
        ...
        val rest = if (next.contains("0")) {
          val toCurrentData: FormModel[T] = FormModel.fromPages(basicAddToList(source, index + 1, data))
          val hij: FormModel[T] = mkFormModel(toCurrentData, data)
          hij.pages
        } else {
          Nil
        }

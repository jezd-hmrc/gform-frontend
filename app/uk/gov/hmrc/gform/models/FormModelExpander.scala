...
object FormModelExpander {
  ...
  implicit val interim = new FormModelExpander[Interim] {
    ...
    def liftRepeating(
      ...
      val repeats = if (indexes.isEmpty) 1 else indexes.max

      val pageBasic = mkSingleton2(section.page, repeats)(section)
      Singleton(pageBasic.asInstanceOf[Page[Interim]], section) :: Nil
    }
  }

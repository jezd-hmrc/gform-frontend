...
import uk.gov.hmrc.gform.eval.{ EvaluationResults, ExprType, ExpressionResult, TypedExpr }
...
class FormModelSpec extends FlatSpec with Matchers with FormModelSupport with VariadicFormDataSupport {
  ...
  "FormModel" should "bla bla" in {
    ...
    val table = Table(
      ("data", "expected"),
      (variadicFormDataMany(), rc :: Nil),
      (variadicFormDataMany("rc" -> List(0)), rc :: fcA :: Nil),
      (variadicFormDataMany("rc" -> List(1)), rc :: fcB :: Nil),
      (variadicFormDataMany("rc" -> List(0, 1)), rc :: fcA :: fcB :: Nil)
    )

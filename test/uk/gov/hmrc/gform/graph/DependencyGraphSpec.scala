...
import uk.gov.hmrc.gform.models.{ Basic, DependencyGraphVerification, FormModel, FormModelSupport, SectionSelectorType, VariadicFormDataSupport }
...
class DependencyGraphSpec extends FlatSpec with Matchers with FormModelSupport with VariadicFormDataSupport {
  ...
  it should "detect cyclic cross reference in section" in {
    ...
    thrown.graphNode shouldBe GraphNode.Expr(FormCtx("b"))
  ...
  val nonRepeatedSectionValidators: TableFor2[String, Page[Basic]] = {
    ...
    val bankAccoutnModulusCheck = BankAccoutnModulusCheck(
      toSmartString(""),
      FormCtx(""),
      FormCtx("")
    )
    ...
    def bankAccount(bank: BankAccoutnModulusCheck): Page[Basic] =
    ...
    Table(
      ...
      ("validators.BankAccoutnModulusCheck.errorMessage",  bankAccount(bankAccoutnModulusCheck.copy(errorMessage = stringExpr))),
      ("validators.BankAccoutnModulusCheck.accountNumber", bankAccount(bankAccoutnModulusCheck.copy(accountNumber = ctx))),
      ("validators.BankAccoutnModulusCheck.sortCode",      bankAccount(bankAccoutnModulusCheck.copy(accountNumber = ctx)))
      // format: on
    )
  }
  ...
  val formTemplateExpressionTable = {
    ...
    val emptyAcknowledgementSection = AcknowledgementSection(emptySS, None, None, List.empty[FormComponent], true)
    val emptyHmrcDms = Destination.HmrcDms(DestinationId(""), "", Value, "", "", "", false, false)
  ...
  private def layers(formTemplate: FormTemplate)(implicit position: Position): List[(Int, Set[GraphNode])] = {
    ...
    DependencyGraph.constructDependencyGraph(DependencyGraph.toGraphFull(fm, formTemplateExprs)) match {

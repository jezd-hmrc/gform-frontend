 ...
 *   }
 *
 *   it should "return an empty string when authModule in authConfig is set anonymous" in new UserCtxEvaluatorProcessor {
 ...
 *   }
 *
 *   it should "return enrolledIdentifier value when authModule in authConfig are set" in new UserCtxEvaluatorProcessor {
 ...
 *   lazy val nonConvertible: String => NonConvertible = value => NonConvertible(RecalculationOp.newValue(value).pure[Id])
